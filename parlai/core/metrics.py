...
def dimen(sents, weights=(0.25, 0.25, 0.25, 0.25)):
    """
    DIversity MEtric based on N-grams.
    :return:
    """
    res = {}
    weights = np.array(weights)
    n_grams = [Counter() for _ in range(len(weights))]
    s_dimen = np.array([])
    for sent in sents:
        div_score = 0.0
        for i, w in enumerate(weights, start=1):
            counts = Counter(ngrams(sent, i, pad_right=True))
            div_score += w * len(counts) / sum(counts.values()) # diversity scores for each order of current sentence
            n_grams[i-1].update(counts) # used to calculate corpus-level dimen scores

        s_dimen = np.append(s_dimen, div_score) # dimen scores of this sentence

    c_dimen = 0.0 # corpus-level dimen score
    for i, (i_gram, w) in enumerate(zip(n_grams, weights)):
        c_dimen += w * len(i_gram) / sum(i_gram.values())
        key = '#d-{}'.format(i+1)
        res[key] = len(i_gram)
    res['#tok'] = sum(n_grams[0].values())
    res['c_dimen'] = c_dimen

    for k, v in res.items():
        res[k] = round_sigfigs(v, 4)
