...
from parlai.core.metrics import dimen
from parlai.utils.misc import round_sigfigs, warn_once
from parlai.utils.torch import padded_tensor, neginf, NEAR_INF
...
class TorchGeneratorAgent(TorchAgent):
    ...
    def add_cmdline_args(cls, argparser):
        ...
        agent.add_argument(
            ...
            type='bool',
            default=False,
            help='If True, allow model to output __UNK__ token.',
        )
        agent.add_argument(
            '--gamma',
            type=float,
        ...
        agent.add_argument(
            ...
            type=int,
            default=10,
            help='Window size of history repetition rates.',
        )
        agent.add_argument(
            '--weight-level',
            choices={'token', 'sequence'},
            default='token',
            help='At what level to apply the loss weights.',
        )
        agent.add_argument(
            '--weight-func',
            choices={'fl', 'ldr'},
            default='ldr',
            help='Which weighting funciton to use.',
        )
    ...
    def __init__(self, opt: Opt, shared=None):
        ...
        self.bins = opt.get('bins')
        self.hist_weights = np.arange(self.bins - 1, -1, -1) / self.bins
        self.allow_unknown = opt.get('allow_unknown')
        self.unknown_idx = self.dict[self.dict.unk_token]
        self.gamma = opt.get('gamma')
        self.display_examples = opt.get('display_examples')
        self.window_sz = opt.get('repeat_window_size')
        self.repeat_window = deque(maxlen=self.window_sz) # keep track of a slide window of valid repetition scores
        self.weight_level = opt.get('weight_level')
        self.weight_func = opt.get('weight_func')
    ...
    def report(self):
        ...
        if not self.is_training and self.metrics['preds']:
            div, s_dimen = dimen(self.metrics['preds'])
            _, h_dimen = dimen(self.metrics['gt'])
            # if not self.model.training:
            base.update(div)
            sys_rate, _ = self.repetition_rate(s_dimen, h_dimen)
            base['repeat'] = round_sigfigs(sys_rate, 4)
            self.repeat_window.append(sys_rate)
            base['repeat_mean'] = round_sigfigs(np.mean(self.repeat_window), 4)
            base['repeat_std'] = round_sigfigs(np.std(self.repeat_window), 4)
        return base

    def repetition_rate(self, s_dimen, h_dimen):
        sys_hist, _ = np.histogram(s_dimen, bins=self.bins, range=(0, 1))
        hu_hist, _ = np.histogram(h_dimen, bins=self.bins, range=(0, 1))
        sys = norm(sys_hist * self.hist_weights)
        human = norm(hu_hist * self.hist_weights)
        if not self.model.training:
            print("System:")
            for i in sys_hist:
                print(i)
            ...
            for i in hu_hist:
                print(i)
            print('Human repetition: {}'.format(human))
        return sys, human
    ...
    def compute_loss(self, batch, return_output=False):
        ...
        raw_loss = self.criterion(score_view, batch.label_vec.view(-1))
        if self.is_training:
            labels = batch.label_vec.view(-1)
            one_hot = nn.functional.one_hot(labels, scores.size(-1)).bool()
            ps = score_view.softmax(-1).masked_select(one_hot)
            if self.weight_level == 'token':
                loss = self.loss_weight(ps).dot(raw_loss)
            else:
                ps = ps.reshape(preds.size())
                label_length = torch.tensor(batch.label_lengths).to(ps.device) if batch.label_lengths is not None else 1
                mean_ps = ps.sum(dim=-1).div(label_length)
                raw_loss = raw_loss.reshape(preds.size())
                loss = self.loss_weight(mean_ps).matmul(raw_loss).sum()
        else:
            loss = raw_loss.sum()
        ...
        self.metrics['nll_loss'] += raw_loss.sum().item()
        self.metrics['num_tokens'] += target_tokens
        if self.model.training:
            self.metrics['preds'].extend(self._clean_preds(preds))
            self.metrics['gt'].extend(self._clean_preds(batch.label_vec))
    ...
    def loss_weight(self, ps):
        # cos(x * pi) + 1
        if self.weight_func == 'ldr':
            weights = (ps * np.pi).cos() + 1
        elif self.weight_func == 'fl':
            weights = (1 - ps).pow(self.gamma)
    ...
    def eval_step(self, batch):
        ...
        preds_clean = self._clean_preds(preds)
        self.metrics['preds'].extend(preds_clean)
        self.metrics['gt'].extend(self._clean_preds(batch.label_vec))
        if self.display_examples:
            _, s_dimen = dimen(preds_clean)
            s_dimen = s_dimen.round(2)
            text = ['{} s_dimen: {}'.format(self._v2t(p), d) for p, d in zip(preds, s_dimen)] if preds is not None else None
        else:
    ...
    def _generate(self, batch, beam_size, max_ts):
        ...
        for _ts in range(max_ts):
            ...
            if not self.allow_unknown:
                score[:, :, self.unknown_idx] = -NEAR_INF
