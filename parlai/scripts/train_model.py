...
def setup_args(parser=None) -> ParlaiParser:
    ...
    train.add_argument(
        ...
        type=int,
        default=10,
        help='Number of validations to skip saving the model, for less noisy checkpoints.',
    )
    train.add_argument(
        '-veps',
...
def _save_best_valid(model_file, best_valid, best_repeat=None):
    """
    Save the best validation score to disk.
    """
    f = open(model_file + '.best_valid', 'w')
    if best_repeat is not None:
        f.write('{}\t{}'.format(best_valid, best_repeat))
    else:
...
class TrainLoop:
    ...
    def __init__(self, opt):
        ...
        if opt.get('validation_metric_mode') is None:
            opt['validation_metric_mode'] = 'max'
        if opt['validation_metric'] == 'repeat':
            self.best_mean_repeat = float('inf')
        else:
            self.best_mean_repeat = None
        ...
        if opt.get('model_file') and os.path.isfile(opt['model_file'] + '.best_valid'):
            with open(opt['model_file'] + ".best_valid", 'r') as f:
                x = f.readline()
                if opt['validation_metric'] == 'repeat':
                    repeat, mean_repeat = x.split()
                    self.best_valid = float(repeat)
                    self.best_mean_repeat = float(mean_repeat)
                else:
    ...
    def validate(self):
        ...
        # logging
        if opt['tensorboard_log'] and is_primary_worker():
            self.tb_logger.log_metrics('valid', valid_report['total_train_updates'], valid_report)
        # saving
        if (
            opt.get('model_file')
        ...
        mean_repeat = valid_report['repeat_mean']
        self.validate_count += 1

        # check if this is the best validation so far
        if self.validate_count < opt['skip_saving']:
            print('Skip saving for {} validations. Currently {}'.format(opt['skip_saving'], self.validate_count))
        elif (
            self.best_valid is None
            or self.valid_optim * new_valid >= self.valid_optim * self.best_valid
            and self.better_repeat(opt['validation_metric'] == 'repeat', new_valid, mean_repeat)
        ):
            print(
                '[ new best {}: {}{} ]'.format(
            ...
            if opt['validation_metric'] == 'repeat':
                self.best_mean_repeat = mean_repeat
            self.impatience = 0
            if opt.get('model_file') and is_primary_worker():
                ...
                _save_best_valid(opt['model_file'], self.best_valid, self.best_mean_repeat)
                self.saved = True
            if (
    ...
    def better_repeat(self, execute, new_valid, mean_repeat):
        if execute:
            if mean_repeat > self.best_mean_repeat:
                print('NOT saving due to repetition rate {} is lower than previous {},'
                      ' but mean repetition rate {} is higher than {}.'.format(
                    new_valid, self.best_valid,
                    mean_repeat, self.best_mean_repeat,
                ))
                return False
            else:
                return True
    ...
    def log(self):
        ...
        if opt['tensorboard_log'] and is_primary_worker():
            self.tb_logger.log_metrics('train', train_report['total_train_updates'], train_report)
