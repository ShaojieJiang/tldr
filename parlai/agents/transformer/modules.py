...
from parlai.utils.torch import neginf, NEAR_INF
...
def _normalize(tensor, norm_layer, before=False, after=False, embedding=False):
    """
    Broadcast layer norm.
    """
    if before | after | embedding:
        size = tensor.size()
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, input, positions=None, segments=None):
        ...
        if self.variant == 'xlm':
            tensor = _normalize(tensor, self.norm_embeddings, embedding=True)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, tensor, mask):
        """Forward pass."""
        residual = tensor
        tensor = _normalize(tensor, self.norm1, before=self.before)
        tensor = residual + self.dropout(self.attention(tensor, mask=mask))
        tensor = _normalize(tensor, self.norm1, after=self.after)
        residual = tensor
        tensor = _normalize(tensor, self.norm2, before=self.before)
        tensor = residual + self.dropout(self.ffn(tensor))
        tensor = _normalize(tensor, self.norm2, after=self.after)
        tensor *= mask.unsqueeze(-1).type_as(tensor)
        return tensor
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x, encoder_output, encoder_mask):
        ...
        # first self attn
        residual = x
        x = _normalize(x, self.norm1, before=self.before)
        # don't peak into the future!
        x = self.self_attention(query=x, mask=decoder_mask)
        x = self.dropout(x)  # --dropout
        x = x + residual
        x = _normalize(x, self.norm1, after=self.after)

        residual = x
        x = _normalize(x, self.norm2, before=self.before)
        x = self.encoder_attention(
            query=x, key=encoder_output, value=encoder_output, mask=encoder_mask
        )
        x = self.dropout(x)  # --dropout
        x = residual + x
        x = _normalize(x, self.norm2, after=self.after)

        # finally the ffn
        residual = x
        x = _normalize(x, self.norm3, before=self.before)
        x = self.ffn(x)
        x = self.dropout(x)  # --dropout
        x = residual + x
        x = _normalize(x, self.norm3, after=self.after)
