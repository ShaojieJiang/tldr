...
try:
    ...
    raise ImportError(
        "Please install HuggingFace Transformers from"
        "https://github.com/huggingface/transformers"
    )

from .module import TransformerModel, HfDictionaryAgent

from parlai.core.torch_generator_agent import TorchGeneratorAgent
from parlai.agents.seq2seq.modules import opt_to_kwargs


class HfTransformerAgent(TorchGeneratorAgent):
    """"""

    @classmethod
    def add_cmdline_args(cls, argparser):
        """Add command-line arguments specifically for this agent."""
        agent = argparser.add_argument_group('HfTransformer Arguments')
        agent.add_argument(
            '-esz',
            '--embedding-size',
            type=int,
        ...
        agent.add_argument(
            '--activation',
            choices={'relu', 'gelu', 'swish', 'gelu_new'},
            default='relu',
        ...
        agent.add_argument(
            '--share-lt',
            choices={'enc_dec', 'dec_out', 'dec_out_copy', 'unique', 'all'},
            default='dec_out_copy',
        ...
        agent.add_argument(
            ...
            type='bool',
            default=False,
            help='Whether to use HF pretrained model parameters.'
            'When set to True, all the specified model parameters'
            "don't matter anymore.",
        )

        super(HfTransformerAgent, cls).add_cmdline_args(argparser)
        return agent

    @staticmethod
    def model_version():
        # allowing using pretrained model makes it version 0.2!
        return 0.2

    @staticmethod
    def dictionary_class():
        return HfDictionaryAgent

    def __init__(self, opt, shared=None):
        super().__init__(opt, shared)
        self.id = 'HfTransformer'
    ...
    def build_model(self, states=None):
        ...
        model = TransformerModel(
            ...
            opt['embedding_size'],
            opt['ffn_size'],
            use_pretrained=opt['load_pretrained'],
            nlayers=opt['n_layers'],
            nheads=opt['n_heads'],
            npos=opt['n_positions'],
            dropout=opt['dropout'],
            lookuptable=opt['share_lt'],
            attn_drop=opt['attention_dropout'],
        ...
        if opt['embedding_type'].endswith('fixed'):
            print('HfTransformer: fixing embedding weights.')
