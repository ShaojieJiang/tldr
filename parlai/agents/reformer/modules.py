...
def _build_decoder(
    opt, dictionary, embedding=None, padding_idx=None, n_positions=1024, n_segments=0, rnn_class='gru',
    rnn_layers=1,
    recurrence='none',
):
...
class ReformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        elif recurrence == 'rnn': # share RNN parameters among all layers
            shared_rnn = ReformerRNN(embedding_size, hiddensize, num_layers=rnn_layers, rnn_class=rnn_class, batch_first=True, dropout=dropout)
...
class ReformerEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if shared_rnn:
            ...
            self.rnn = ReformerRNN(embedding_size, hiddensize, num_layers=rnn_layers, rnn_class=rnn_class, batch_first=True, dropout=dropout)
...
class ReformerRNN(nn.Module):
    ...
    def __init__(self, embedding_size, hiddensize, num_layers=1, rnn_class='gru', batch_first=True, dropout=0.0):
        super(ReformerRNN, self).__init__()
        rnn_class = ReformerRNN.RNN_OPTS[rnn_class]
        self.rnn = rnn_class(embedding_size, hiddensize, num_layers=num_layers, batch_first=batch_first)
        self.emb_size = embedding_size
        self.hid_size = hiddensize
        self.num_layers = num_layers
        self.batch_first = batch_first
        self.linear = nn.Linear(hiddensize, embedding_size)
